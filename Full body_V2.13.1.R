# Based on V 2.13
# Inverse the dc/dt for myo/other
...
{
  t_end <- 50 #[h] time of the end of simulation
  ...
  Vlu <- ((29.08 * (height/ 100) * weight ^ 0.5 + 11.06 + 35.47 * (height/100) * weight^0.5 +5.53)/1000)/1.05
  Vad <- (1.36 * weight) / (height/ 100) - 42 #adipose (>12y)
  Vad[Vad<0] <- 0.01
  Vad <- ifelse((age >12.3), Vad, 0)
  
  Vhe <- (( 22.81 * (height/100) * weight^0.5 -4.15)/1000)/1.05
  Vki <- (4.214 * weight^0.823 + 4.456*weight^0.795)/1000
  Vmu <- (0.3 + ((0.54-0.3)/18)* age) * (weight - Vad * 0.92)/1.04
  Vsk <- (BSA/1000) * 45.655+ (BSA/1000) * 1240
  Vsp <- ((8.74 * (height /100) * weight^0.5 + 11.06)/1000)*1.06
  Vgu <- (0.021* (weight - Vad * 0.92))/1.05
  Vli <- ((576.9*(height/100) + 8.9*weight - 159.7)/1000)/1.05
