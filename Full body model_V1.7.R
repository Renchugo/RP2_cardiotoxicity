...
require(ggquickeda)
{
  t_end <- 2 #[h] time of the end of simulation
  times <- seq(0, t_end, by = 0.001) #time of simulation
...

{
  ...
  cell_concentration_endo <- cell_amount_endo / Vhe#endomyocardial
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      Cendo_ict <- Aendo_ict / Vhe #heart endo intracellular total concentration
      Cmid_ict <- Amid_ict / Vhe #heart mid intracellular total concentration
      Cepi_ict <- Aepi_ict / Vhe #heart epi intracellular total concentration
      
      # Heart extracellular sub-compartment
      dAheart_ec <-  Qhe * (Carterial - (Cheart_ec/Kpec) * BP) - 0.001 * PER * SA_endo * (Cheart_ec * fu_ec * funionized_ec - Cendo_icfree/(Kp_endo * Kpp) * funionized_ic ) -0.001 * PER * SA_mid * (Cheart_ec * fu_ec * funionized_ec - Cmid_icfree/(Kp_mid * Kpp) * funionized_ic ) - 0.001 *PER * SA_epi * (Cheart_ec * fu_ec * funionized_ec - Cepi_icfree/(Kp_epi * Kpp) * funionized_ic )
     
      # Subcompartments intracellular total concentration
      dAendo_ict <-  0.001 * PER * SA_endo * (Cheart_ec * fu_ec * funionized_ec - Cendo_icfree/(Kp_endo * Kpp) * funionized_ic ) - Cendo_ict * fu_heart * CLint_heart * (Vendo_ic/Vhe)
      dAmid_ict <-   0.001 * PER * SA_mid * (Cheart_ec * fu_ec * funionized_ec - Cmid_icfree/(Kp_mid * Kpp) * funionized_ic ) - Cmid_ict * fu_heart * CLint_heart * (Vmid_ic/Vhe)
      dAepi_ict <-   0.001 * PER * SA_epi * (Cheart_ec * fu_ec * funionized_ec - Cepi_icfree/(Kp_epi * Kpp) * funionized_ic ) - Cepi_ict * fu_heart * CLint_heart * (Vepi_ic/Vhe)
      
      # Nuclear free sub-compartment (first bind with DNA/cardiolipin)
      Cendo_ic <- 0.5 * (sqrt((Cendo_ict + DNA_endo + Kd_DNA )^2 - 4* DNA_endo *Cendo_ict) + sqrt((Cendo_ict + cardiolipin + Kd_cardiolipin )^2 - 4* cardiolipin *Cendo_ict) - (DNA_endo + Kd_DNA + cardiolipin + Kd_cardiolipin) )
      Cmid_ic <- 0.5 * (sqrt((Cmid_ict + DNA_mid + Kd_DNA )^2 - 4* DNA_mid *Cmid_ict) + sqrt((Cmid_ict + cardiolipin + Kd_cardiolipin )^2 - 4* cardiolipin *Cmid_ict) - (DNA_mid + Kd_DNA + cardiolipin + Kd_cardiolipin) )
      Cepi_ic <- 0.5 * (sqrt((Cepi_ict + DNA_epi + Kd_DNA )^2 - 4* DNA_epi *Cepi_ict) + sqrt((Cepi_ict + cardiolipin + Kd_cardiolipin )^2 - 4* cardiolipin *Cepi_ict) - (DNA_epi + Kd_DNA + cardiolipin + Kd_cardiolipin) )
      
      # Mitochondrial bound concentration (second bind with mtDNA)
      Cendo_icbound <- 0.5 * ((Cendo_ic - mtDNA_endo - Kd_mtDNA) + sqrt((Cendo_ic + mtDNA_endo + Kd_mtDNA)^2 - 4* mtDNA_endo * Cendo_ic))
      Cmid_icbound <- 0.5 * ((Cmid_ic - mtDNA_mid - Kd_mtDNA) + sqrt((Cmid_ic + mtDNA_mid + Kd_mtDNA)^2 - 4* mtDNA_mid * Cmid_ic))
      Cepi_icbound <- 0.5 * ((Cepi_ic - mtDNA_epi - Kd_mtDNA) + sqrt((Cepi_ic + mtDNA_epi + Kd_mtDNA)^2 - 4* mtDNA_epi * Cepi_ic))
      
      # Mitochondrial free concentration after binding with mtDNA, cardiolipin, DNA
      Cendo_icfree <- Cendo_ic - Cendo_icbound
      Cmid_icfree <- Cmid_ic - Cmid_icbound
      Cepi_icfree <- Cepi_ic - Cepi_icbound
      ...
      list(
        ...
        Cheart = Ahe / Vhe,
        Crest = Are / Vre,
        MID_ict = Amid_ict / Vhe,
        ENDO_ict = Aendo_ict / Vhe,
        EPI_ict = Aepi_ict / Vhe,
        EC = Cheart_ec,
