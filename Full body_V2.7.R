...

{
  ...
  MV <- exp(age * 0.04551 + 7.36346) #[mcm^3]
  MSA <-exp(sqrt(0.102 ^ 2 + (log(MV)) ^ 2 * 0.002939 ^ 2)) #[mcm^2]
  ...
  #cells amounts
  cell_amount_other <- Vother_ic / MVol 
  cell_amount_myo <- Vmyo_ic / MVol 
...
# Calculation kp based on Rodgers and Rowland method
{
  ...
  Vss <- Vpl/ fup + Vad * Kpad + Vbo * Kpbo + Vbr * Kpbr + Vgu * Kpgu + Vhe * Kphe + Vki * Kpki  + Vli * Kpli + Vlu * Kplu + Vmu * Kpmu + Vre * Kppa + Vsk * Kpsk + Vsp * Kpsp
...

{
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      dAve <- inf + Qad * (Cadipose / Kpad * BP) + Qbo * (Cbone / Kpbo * BP) + Qbr * (Cbrain / Kpbr * BP) + Qki* (Ckidney / Kpki * BP) + Qli * (Cliver / Kpli * BP) + Qmu* (Cmuscle / Kpmu * BP) + Qsk* (Cskin / Kpsk * BP)+ Qhe * (Che_ec / Kpec_he * BP) - Qlu * Cvenous #venous blood
      dAar <- Qlu * (Clung / Kplu * BP) - Qad * Carterial - Qbo * Carterial - Qbr * Carterial - Qgu * Carterial- Qki * Carterial- Qha * Carterial- Qmu * Carterial- Qsk * Carterial- Qsp * Carterial 	#arterial blood
      ...
      dCother_ict <- 0.001 * PER * SA_other * (Che_ec * fuec_he * funionized_ec - Cother_ict/(Kpec_he * Kpp) * funionized_ic ) / Vother_ic 
      dCmyo_ict <- 0.001 * PER * SA_myo * (Che_ec * fuec_he * funionized_ec - Cmyo_ict/(Kpec_he * Kpp) * funionized_ic ) / Vmyo_ic 
