...
ModelVar <- function (BW,
  ...
  mtDNA_epi <- 0.2 * 90.6 * 0.01
  mtDNA_mid <- 0.3 * 90.6 * 0.01
  mtDNA_endo <- 0.5 * 90.6 * 0.01 
  ...
  BP <- 0.74533 #blood to plasma ratio from Simcyp
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      Cliverfree <- Cliver * (fup / BP) * fuha  #liver_free concentration
      Ckidneyfree <-Ckidney * (fup / BP)	 #kidney_free concentration
      ...
      dAli <- Qha * Carterial + Qgu * (Cgut / Kpgu * BP) + Qsp * (Cspleen / Kpsp * BP) - Qli * (Cliver / Kpli * BP) - Cliverfree * CL_hepatic #liver
