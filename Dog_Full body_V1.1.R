...
{  #Healthy beagles-------------------------------------------------------------------
  N <- 100
  rn <- seq.default(from = 1, to = N, by = 1)
  age <- runif(N,1,15) #years
  weight <- runif(N,8,15) #[kg] 
  height <- runif(N,30,41) #[cm]
  CO_range <- runif(N,7.2,12) #per kilogram BW [ml/kg/min] to [L/kg/h], https://doi.org/10.1016/0002-8703(71)90054-8
  CL_range <- runif(N,13.5,20.9) # [ml/min/kg] Baldwin et al 1992 
  ...
  pop["CO"] <- pop$weight * pop$CO_var
  pop["BSA"] <- 0.101 * (pop$weight^(2/3))
  pop["CL_sys"] <- pop$CL_range * pop$weight * 60 / 1000 #ml/min to l/h
  
  #BLOOD FLOWS for Beagles according to Simcyp
  # BLOOD FLOWS [L/h] -------------------------------------------------------
  pop["Qad"] <-
    pop$CO  * 0.065  #Adipose
  pop["Qbo"] <-
    pop$CO  * 0.079  #Bone
  pop["Qbr"] <-
    pop$CO  * 0.028  #Brain
  pop["Qso"] <- 
    pop$CO  * 0.029  #Stomach and oesophagus
  pop["Qsi"] <- 
    pop$CO  * 0.084  #Small intestine
  pop["Qla"] <- 
    pop$CO  * 0.024  #Large intestine
  pop["Qhe"] <-
    pop$CO  * 0.044  #Heart
  pop["Qsk"] <-
    pop$CO  * 0.068  #Skin
  ...
  pop["Qki"] <-
    pop$CO  * 0.159  #Kidney
  pop["Qha"] <- 
    pop$CO  * 0.07   #Hepatic artery
  pop["Qhp"] <-
    pop$CO  * 0.204  #Hepatic portal vein
  pop["Qpa"] <-
    pop$CO  * 0.023  #Pancreas
  pop["Qlu"] <-
    pop$CO   #lung
  pop["Qgu"] <-
    pop$Qso + pop$Qsi + pop$Qla #Gut
  pop["Qli"] <- pop$Qha + pop$Qsp + pop$Qpa + pop$Qgu  #Liver
  pop["Qsubtot"] <- 
    pop$Qad + 
    pop$Qbo + 
    pop$Qbr +
  ...
  pop["Vad"] <- (0.2247 * pop$weight)
  pop["Vbo"] <- (0.0435 * pop$weight)
  pop["Vbr"] <- (0.0062 * pop$weight)
  pop["Vgu"] <- (0.0417 * pop$weight)
  pop["Vhe"] <- (0.0070 * pop$weight)
  pop["Vki"] <- (0.0044 * pop$weight)
  pop["Vli"] <- (0.0278 * pop$weight)
  pop["Vlu"] <- (0.0080 * pop$weight)
  pop["Vmu"] <- (0.3910 * pop$weight)
  pop["Vsk"] <- (0.0985 * pop$weight)
  pop["Vsp"] <- (0.0026 * pop$weight)
  pop["Vpa"] <- (0.0023 * pop$weight)
  pop["Vpl"] <- (0.0443 * pop$weight)
  pop["Vrb"] <- (0.0350 * pop$weight)
  pop["Vbl"] <- pop$Vpl + pop$Vrb
  pop["Vsubtot"] <-
    pop$Vad +
    pop$Vbo +
    pop$Vbr +
  ...
  pop["Vre"] <- pop$weight * body_density - pop$Vsubtot
  pop["Vtot"] <- pop$Vre + pop$Vsubtot
    
  #intracellular [ic] and extracellular [ec] fractions [L] per organ according to PK sim
  pop["Vad_ic"] <- pop$Vad * 0.86
  pop["Vad_ec"] <- pop$Vad * 0.14
  
  pop["Vbo_ic"] <- pop$Vbo * 0.86
  pop["Vbo_ec"] <- pop$Vbo * 0.14
  
  pop["Vbr_ic"] <- pop$Vbr * 0.96
  pop["Vbr_ec"] <- pop$Vbr * 0.04
  
  pop["Vgu_ic"] <- pop$Vgu * 0.87
  pop["Vgu_ec"] <- pop$Vgu * 0.13
  
  pop["Vhe_ic"] <- pop$Vhe * 0.64
  pop["Vhe_ec"] <- pop$Vhe * 0.36
  
  pop["Vki_ic"] <- pop$Vki * 0.7
  pop["Vki_ec"] <- pop$Vki * 0.3
  
  pop["Vli_ic"] <- pop$Vli * 0.72
  pop["Vli_ec"] <- pop$Vli * 0.28
  
  pop["Vlu_ic"] <- pop$Vlu * 0.19
  pop["Vlu_ec"] <- pop$Vlu * 0.81
  
  pop["Vmu_ic"] <- pop$Vmu * 0.85
  pop["Vmu_ec"] <- pop$Vmu * 0.15
  
  pop["Vsk_ic"] <- pop$Vsk * 0.68
  pop["Vsk_ec"] <- pop$Vsk * 0.32
  
  pop["Vsp_ic"] <- pop$Vsp * 0.57
  pop["Vsp_ec"] <- pop$Vsp * 0.43
  
  pop["Vpa_ic"] <- pop$Vpa * 0.7
  pop["Vpa_ec"] <- pop$Vpa * 0.3
  ...
  pop["Vre_ic"] <- pop$Vre * 0.72 #average ic
  pop["Vre_ec"] <- pop$Vre * 0.28 #average ec
  
  #Tissue volumes [L] -------------------------------------------------------
  pop["Vmyo"] = 0.8 * pop$Vhe #myocardial
  pop["Vother"] = 0.2 * pop$Vhe #other
  pop["Vmyo_ic"] = 87.5 / 100 * pop$Vmyo #intracellular of myocardial
  pop["Vother_ic"] = 87.5 / 100 * pop$Vother #intracellular 
  pop["Vhe_ec"] = 12.5 / 100 * pop$Vhe #extracellular space of heart tissue
  pop["Vve"] = (2 / 3) * pop$Vbl		#venous blood; assumed 2/3 of total blood according to volmues published in CPT. Regarding the distribution of blood volume within the circulation, the greatest volume resides in the venous vasculature, where 70-80% of the blood volume is found. -> http://www.cvphysiology.com/Blood%20Pressure/BP019
  pop["Var"] = pop$Vbl - pop$Vve		#arterial blood
  pop["Vplas_ven"] = pop$Vpl * (pop$Vve / (pop$Vve + pop$Var))  #venous plasma
  pop["Vplas_art"] = pop$Vpl * (pop$Var / (pop$Vve + pop$Var)) 	#arterial plasma
  ...
  #based on assumption that 1% of total DNA is mtDNA
    mtDNA_li <- 0.01 * DNA_li
    mtDNA_he <- 0.01 * DNA_he  
    mtDNA_ki <- 0.01 * DNA_ki 
    mtDNA_bo <- 0.01 * DNA_bo 
    mtDNA_gu <- 0.01 * DNA_gu 
    
    mtDNA_mu <- 0.01 * DNA_mu 
    mtDNA_ad <- 0.01 * DNA_ad 
    mtDNA_sk <- 0.01 * DNA_sk 
    
    mtDNA_br <- 0.01 * DNA_br 
    mtDNA_lu <- 0.01 * DNA_lu 
    mtDNA_sp <- 0.01 * DNA_sp 
    mtDNA_pa <- 0.01 * DNA_pa
    mtDNA_re <- 0.01 * DNA_re 
...
#CARDIOMYOCYTE VOLUME AND SURFACE AREA -------------------------------------------------------
  ...
  #cells amounts -------------------------------------------------------
  pop["cell_amount_other"] <- pop$Vother_ic / MVol #other heart tissue
  pop["cell_amount_myo"] <- pop$Vmyo_ic / MVol #myocardial
  
  pop["SA_other"] <- (pop$cell_amount_other * MSA) / (10^8) #[cm^2]
  pop["SA_myo"] <- (pop$cell_amount_myo * MSA) / (10^8) #[cm^2]
  ...
  #population dose
  pop["inf_dose"] <- inf_dose_mg * pop$BSA * (0.001 / MW ) * 1000000 # [umol]
  #pop["inf_dose"] <- inf_dose_mg * pop$weight * (0.001 / MW ) * 1000000 # [umol]
  
  #IV INFUSION RATE
  pop["inf"] = (pop$inf_dose/inf_time) #[umol/h]
  #DISTRIBUTION  -------------------------------------------------------
  #Drug binding
  ...
  #pop["CL_renal"] <- 1.06 * pop$weight * 60 / 1000 #[L/ h] renal clearance
  #CL_hepatic <- 29.97 #[L/ h] hepatic clearance
  CLint_heart <- 0 #[L/ h] heart clearance CYP3A4 was not detected in heart tissue
  #pop["CL_hepatic"] <-runif(N,15,80) #[L/h] Systemic clearance
  
  
}  
    
  pop <- pop[order(pop$rn),]
  pop <- pop[order(as.numeric(row.names(pop))),]
  setDT(pop)
  pop$rn <- as.numeric(as.character(pop$rn))
...

 {
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      ## rates of changes
      dINFUSION <- -inf
      dCve <- (inf - CL_sys + Qad * (Cad_ec / Kpad * BP) + Qbo * (Cbo_ec / Kpbo * BP) + Qbr * (Cbr_ec / Kpbr * BP) + Qki* (Cki_ec / Kpki * BP) + Qli * (Cli_ec / Kpli * BP) + Qmu* (Cmu_ec / Kpmu * BP) + Qsk* (Csk_ec / Kpsk * BP)+ Qhe * (Che_ec / Kphe * BP) + Qre * (Cre_ec / Kpre * BP) - Qlu * Cve - PER * SA_bloodcell * 0.001 * (Cve* funionized_ec - (Cbloodcell/Kpp)* funionized_ic))/ Vve #venous blood
      ...
      dCpa_ec <- (Qpa * (Car - (Cpa_ec/Kppa) * BP) - (0.001 * PER * SA_pa * (Cpa_ec * fuec_pa * funionized_ec - Cpa_ict/(Kpec_pa * Kpp) * funionized_ic)))/ Vpa_ec
      ...
      dCpa_ict <- (0.001 * PER * SA_pa * ((Cpa_ec * fuec_pa * funionized_ec - Cpa_ict/(Kpec_pa * Kpp) * funionized_ic)) / Vpa_ic) - (kon1 * Cpa_ict * Cpa_E1 - koff1 * Cpa_E1R) - (kon2 * Cpa_ict * Cpa_E2 - koff2 * Cpa_E2R) - (kon3 * Cpa_ict * Cpa_E3 - koff3 * Cpa_E3R) 
