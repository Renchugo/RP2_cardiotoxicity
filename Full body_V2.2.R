...
# replace ict with icfree
# time:h dose:mg L/min/mg/umol 
rm(list=ls(all=TRUE))
...
{
  #Scenario set-------------------------------------------------------
  t_end <- 360 #[minutes] time of the end of simulation
  ...
  inf_time <- 2 #[minute] infusion time
...

{
  ...
  PER <- 0.0756/60  # cm/min （huahe Unpublished）
  ...
  CL_renal <- 0.66/60 #[L/ min] renal clearance
  CL_hepatic <- 29.97/60 #[L/ min] hepatic clearance
  CLint_heart <- 0 #[L/ min] heart clearance CYP3A4 was not detected in heart tissue
  CL_total <- 30.6/60 #[L/ min] total clearance
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      dAli <- Qha * Carterial + Qgu * (Cgut / Kpgu * BP) + Qsp * (Cspleen / Kpsp * BP) + Qpa * (Cpa / Kppa * BP) - Qli * (Cliver / Kpli * BP) - CL_hepatic * Cliverfree #liver
      ...
      dCother_ict <- ((0.001 * PER * SA_other * (Che_ec * fuec_he * funionized_ec - Cother_ict/(Kpec_he * Kpp) * funionized_ic ) - Cother_ict * fu_heart * CLint_heart * (Vother_ic/Vhe) ) / Vother_ic) -(kon1 * Cother_ict * DNA_other - koff1 * Cother_E1R) - (kon2 * Cother_ict * Cardiolipin_other - koff2 * Cother_E2) - (kon3 * Cother_ict * mtDNA_other - koff3 * Cother_E3R) 
      dCmyo_ict <- ((0.001 * PER * SA_myo * (Che_ec * fuec_he * funionized_ec - Cmyo_ict/(Kpec_he * Kpp) * funionized_ic) - Cmyo_ict * fu_heart * CLint_heart * (Vmyo_ic/Vhe) ) / Vmyo_ic) - (kon1 * Cmyo_ict * DNA_myo - koff1 * Cmyo_E1R) - (kon2 * Cmyo_ict * Cardiolipin_myo - koff2 * Cmyo_E2) - (kon3 * Cmyo_ict * mtDNA_myo - koff3 * Cmyo_E3R) 
      dCad_ict <- ((0.001 * PER * SA_ad * (Cad_ec * fuec_ad * funionized_ec - Cad_ict/(Kpec_ad * Kpp) * funionized_ic) ) / Vad_ic) - (kon1 * Cad_ict * DNA_ad - koff1 * Cad_E1R) - (kon2 * Cad_ict * Cardiolipin_ad - koff2 * Cad_E2) - (kon3 * Cad_ict * mtDNA_ad - koff3 * Cad_E3R) 
      dCbo_ict <- ((0.001 * PER * SA_bo * (Cbo_ec * fuec_bo * funionized_ec - Cbo_ict/(Kpec_bo * Kpp) * funionized_ic) ) / Vbo_ic) - (kon1 * Cbo_ict * DNA_bo - koff1 * Cbo_E1R) - (kon2 * Cbo_ict * Cardiolipin_bo - koff2 * Cbo_E2) - (kon3 * Cbo_ict * mtDNA_bo - koff3 * Cbo_E3R) 

      dCbr_ict <- ((0.001 * PER * SA_br * (Cbr_ec * fuec_br * funionized_ec - Cbr_ict/(Kpec_br * Kpp) * funionized_ic) ) / Vbr_ic) - (kon1 * Cbr_ict * DNA_br - koff1 * Cbr_E1R) - (kon2 * Cbr_ict * Cardiolipin_br - koff2 * Cbr_E2) - (kon3 * Cbr_ict * mtDNA_br - koff3 * Cbr_E3R) 

      dCgu_ict <- ((0.001 * PER * SA_gu * (Cgu_ec * fuec_gu * funionized_ec - Cgu_ict/(Kpec_gu * Kpp) * funionized_ic) ) / Vgu_ic) - (kon1 * Cgu_ict * DNA_gu - koff1 * Cgu_E1R) - (kon2 * Cgu_ict * Cardiolipin_gu - koff2 * Cgu_E2) - (kon3 * Cgu_ict * mtDNA_gu - koff3 * Cgu_E3R) 

      dCki_ict <- ((0.001 * PER * SA_ki * (Cki_ec * fuec_ki * funionized_ec - Cki_ict/(Kpec_ki * Kpp) * funionized_ic) ) / Vki_ic) - (kon1 * Cki_ict * DNA_ki - koff1 * Cki_E1R) - (kon2 * Cki_ict * Cardiolipin_ki - koff2 * Cki_E2) - (kon3 * Cki_ict * mtDNA_ki - koff3 * Cki_E3R) 

      dCli_ict <- ((0.001 * PER * SA_li * (Cli_ec * fuec_li * funionized_ec - Cli_ict/(Kpec_li * Kpp) * funionized_ic) ) / Vli_ic) - (kon1 * Cli_ict * DNA_li - koff1 * Cli_E1R) - (kon2 * Cli_ict * Cardiolipin_li - koff2 * Cli_E2) - (kon3 * Cli_ict * mtDNA_li - koff3 * Cli_E3R) 

      dClu_ict <- ((0.001 * PER * SA_lu * (Clu_ec * fuec_lu * funionized_ec - Clu_ict/(Kpec_lu * Kpp) * funionized_ic) ) / Vlu_ic) - (kon1 * Clu_ict * DNA_lu - koff1 * Clu_E1R) - (kon2 * Clu_ict * Cardiolipin_lu - koff2 * Clu_E2) - (kon3 * Clu_ict * mtDNA_lu - koff3 * Clu_E3R) 

      dCmu_ict <- ((0.001 * PER * SA_mu * (Cmu_ec * fuec_mu * funionized_ec - Cmu_ict/(Kpec_mu * Kpp) * funionized_ic) ) / Vmu_ic) - (kon1 * Cmu_ict * DNA_mu - koff1 * Cmu_E1R) - (kon2 * Cmu_ict * Cardiolipin_mu - koff2 * Cmu_E2) - (kon3 * Cmu_ict * mtDNA_mu - koff3 * Cmu_E3R) 

      dCsk_ict <- ((0.001 * PER * SA_sk * (Csk_ec * fuec_sk * funionized_ec - Csk_ict/(Kpec_sk * Kpp) * funionized_ic) ) / Vsk_ic) - (kon1 * Csk_ict * DNA_sk - koff1 * Csk_E1R) - (kon2 * Csk_ict * Cardiolipin_sk - koff2 * Csk_E2) - (kon3 * Csk_ict * mtDNA_sk - koff3 * Csk_E3R) 

      dCsp_ict <- ((0.001 * PER * SA_sp * (Csp_ec * fuec_sp * funionized_ec - Csp_ict/(Kpec_sp * Kpp) * funionized_ic) ) / Vsp_ic) - (kon1 * Csp_ict * DNA_sp - koff1 * Csp_E1R) - (kon2 * Csp_ict * Cardiolipin_sp - koff2 * Csp_E2) - (kon3 * Csp_ict * mtDNA_sp - koff3 * Csp_E3R) 

      dCpa_ict <- ((0.001 * PER * SA_pa * (Cpa_ec * fuec_pa * funionized_ec - Cpa_ict/(Kpec_pa * Kpp) * funionized_ic) ) / Vpa_ic) - (kon1 * Cpa_ict * DNA_pa - koff1 * Cpa_E1R) - (kon2 * Cpa_ict * Cardiolipin_pa - koff2 * Cpa_E2) - (kon3 * Cpa_ict * mtDNA_pa - koff3 * Csp_E3R) 
