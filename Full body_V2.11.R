...
require(ggquickeda)
{
  ...
  inf_dose <- inf_dose_mg * 0.001 / MW # mol
...

{
  ...
  DNA_li <- 23.7 * 0.000001 #umol/L 
  DNA_he <- 8.3 * 0.000001  #umol/L 
  DNA_ki <- 16.2 * 0.000001  #umol/L 
  DNA_bo <- 19.1 * 0.000001 #umol/L 
  DNA_gu <- 25.2  * 0.000001 #umol/L 
  
  DNA_mu <- 4.5 * 0.000001  #umol/L Slowly perfused organs
  DNA_ad <- 4.5  * 0.000001 #umol/L 
  DNA_sk <- 4.5 * 0.000001  #umol/L 
  DNA_sp <- 4.5 * 0.000001 #umol/L 
  DNA_pa <- 4.5  * 0.000001 #umol/L 
  DNA_gu <- 4.5  * 0.000001 #umol/L 
  
  DNA_br <- 1.5 * 0.000001  #mol/L Rapidly perfused organ
  DNA_lu <- 1.5 *0.000001  #mol/L 
  
  Cardiolipin_li <- 44.6 *0.000001  #mol/L 
  Cardiolipin_he <- 43.8  * 0.000001 #mol/L 
  Cardiolipin_ki <- 52.3 * 0.000001 #mol/L 
  Cardiolipin_bo <- 25 * 0.000001  #mol/L 
  Cardiolipin_gu <- 25 * 0.000001  #mol/L 
  
  Cardiolipin_ad <- 15 * 0.000001 #mol/L Slowly perfused
  Cardiolipin_mu <- 15 * 0.000001 #mol/L Slowly perfused
  Cardiolipin_sk <- 15 * 0.000001 #mol/L Slowly perfused
  Cardiolipin_sp <- 15 * 0.000001 #mol/L Slowly perfused
  Cardiolipin_pa <- 15 * 0.000001 #mol/L Slowly perfused
  
  Cardiolipin_br <- 30  * 0.000001#mol/L Rapidly perfused
  Cardiolipin_lu <- 30  * 0.000001#mol/L Rapidly perfused
  
  mtDNA_li <- 2.37 * 0.000001 #mol/L assume basesd on DNA concentration
  mtDNA_he <- 8.3  * 0.000001#mol/L 
  mtDNA_ki <- 1.62 * 0.000001 #mol/L 
  mtDNA_bo <- 1.91  * 0.000001#mol/L 
  mtDNA_gu <- 2.52  * 0.000001#mol/L 
  
  mtDNA_mu <- 0.45 * 0.000001 #mol/L
  mtDNA_ad <- 0.45 * 0.000001 #mol/L 
  mtDNA_sk <- 0.45 * 0.000001 #mol/L 
  mtDNA_sp <- 0.45 * 0.000001 #mol/L 
  mtDNA_pa <- 0.45 * 0.000001 #mol/L 
  mtDNA_gu <- 0.45  * 0.000001#mol/L 
  
  mtDNA_br <- 0.15 * 0.000001 #mol/L 
  mtDNA_lu <- 0.15 * 0.000001 #mol/L 
  ...
  Kd_DNA <- 0.00000323 # 3.23 umol/L = 0.00000323 mol/L = 0.00323 mM = 3.23 x 10^-6 M = 3230 nmol/L
  ...
  PBPKModel = function(times, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      dAbo_ec <- Qbo * (Carterial - Cbone / Kpbo * BP) - (0.001 * PER * SA_bo * (Cbo_ec * fuec_bo * funionized_ec - Cbo_ict/(Kpec_bo * Kpp) * funionized_ic) )  
      dAbr_ec <- Qbr * (Carterial - Cbrain / Kpbr * BP) -  (0.001 * PER * SA_br * (Cbr_ec * fuec_br * funionized_ec - Cbr_ict/(Kpec_br * Kpp) * funionized_ic) )
      ...
      dCmyo_ict <- (0.001 * PER * SA_myo * ((Cheart_ec * fu_ec * funionized_ec - Cmyo_ict/(Kpec_he * Kpp) * funionized_ic ) - (Cmyo_ict * fu_heart * CLint_heart * (Vmyo_ic/Vhe))) / Vmyo_ic)  -(kon1 * Cmyo_ict * DNA_myo - koff1 * Cmyo_E1R) - (kon2 * Cmyo_ict * Cardiolipin_myo - koff2 * Cmyo_E2) - (kon3 * Cmyo_ict * mtDNA_myo - koff3 * Cmyo_E3R) 
      dCother_ict <- (0.001 * PER * SA_other * ((Cheart_ec * fu_ec * funionized_ec - Cother_ict/(Kpec_he * Kpp) * funionized_ic) - (Cother_ict * fu_heart * CLint_heart * (Vother_ic/Vhe)))/Vother_ic) - (kon1 * Cother_ict * DNA_other - koff1 * Cother_E1R) - (kon2 * Cother_ict * Cardiolipin_other - koff2 * Cother_E2) - (kon3 * Cother_ict * mtDNA_other - koff3 * Cother_E3R) 
...
plot(
  ...
  col = "blue",
  xlab = "Time [h]",
  ylab = "Blood Concentration [mol/L]"
)  
...
plot(
  ...
  col = "blue",
  xlab = "Time [h]",
  ylab = "Log Blood Concentration [mol/L]"
)  
